...
const firesideBtn = document.querySelector("#fireside");

const forestIco = document.querySelector("#forest svg");
const rainIco = document.querySelector("#rain svg");
const cafeIco = document.querySelector("#cafe svg");
const firesideIco = document.querySelector("#fireside svg");

const forestVolume = document.querySelector("#forest input[type='range']");
const rainVolume = document.querySelector("#rain input[type='range']");
const cafeVolume = document.querySelector("#cafe input[type='range']");
const firesideVolume = document.querySelector("#fireside input[type='range']");

const lightOrDarkMode = document.querySelector(".mode");
let mode = "lightMode";
...
const fireside = new Audio("../sounds/Lareira.wav")
...
function volumeMedium(inputElem, soundName) {
    let ifButtonSoundMiddle = inputElem.value == 1
    let volumeIsNotOn = inputElem.value != 1 && inputElem.value != 2;
    if(ifButtonSoundMiddle) {
        soundName.volume = 0.5
    } else if(volumeIsNotOn) {
        soundName.volume = 0
    }
   
}
function volumeHigh(inputElem, soundName) {
    let ifButtonSoundHigh = inputElem.value == 2
    let volumeIsNotOn = inputElem.value != 1 && inputElem.value != 2;

    if(ifButtonSoundHigh) {
        soundName.volume = 1;
    }  else if(volumeIsNotOn) {
        soundName.volume = 0
    }
...
function soundClickedLightMode(nativeElement, slider, Bro1, Bro2, Bro3) {
    
    nativeElement.querySelector("svg").style.filter = "brightness(1000%)";

    //icones não selecionados ficam preto
    Bro1.querySelector("svg").style.filter = "brightness(0%)";
    Bro2.querySelector("svg").style.filter = "brightness(0%)";
    Bro3.querySelector("svg").style.filter = "brightness(0%)";

    //muda backgroundColor do botao de som, selecionado e o padrão
    nativeElement.style.backgroundColor = "#02799D";
    Bro1.style.backgroundColor = "var(--sound-bg)";
    Bro2.style.backgroundColor = "var(--sound-bg)";
    Bro3.style.backgroundColor = "var(--sound-bg)";

    //muda a cor do slide de volume dependendo do som que está sendo tocado.
    if(slider == forestVolume){
       forestVolume.style.setProperty('--slider-Thumb', `rgb(255, 255, 255)`)
        rainVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        cafeVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        firesideVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`) 
    } else if(slider == rainVolume) {
        rainVolume.style.setProperty('--slider-Thumb', `rgb(255, 255, 255)`)
        forestVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        cafeVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        firesideVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`) 
    } else if( slider == cafeVolume) {
        cafeVolume.style.setProperty('--slider-Thumb', `rgb(255, 255, 255)`)
        firesideVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`) 
        forestVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        rainVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
    } else {
        firesideVolume.style.setProperty('--slider-Thumb', `rgb(255, 255, 255)`)
        rainVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        forestVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
        cafeVolume.style.setProperty('--slider-Thumb', `rgb(50, 50, 56)`)
    }
...
function soundClickedDarkMode(nativeElement, Bro1, Bro2, Bro3) {
    //icone selecionado fica branco, e outros ficam padrão
    nativeElement.querySelector("svg").style.filter = "brightness(1000%)";

    Bro1.querySelector("svg").style.filter = "brightness(400%)";
    Bro2.querySelector("svg").style.filter = "brightness(400%)";
    Bro3.querySelector("svg").style.filter = "brightness(400%)";

    //botao selecionado muda de cor e outros ficam padrão
    nativeElement.style.backgroundColor = "#0A3442";
    Bro1.style.backgroundColor = "#29292E";
    Bro2.style.backgroundColor = "#29292E";
    Bro3.style.backgroundColor = "#29292E";
}

function giveonlyOneSelectedSoundEnfasis(nativeElement, slider, Bro1, Bro2, Bro3) {

    if(mode == "lightMode") {
        soundClickedLightMode(nativeElement, slider, Bro1, Bro2, Bro3)
    } else if(mode == "darkMode") { 
        soundClickedDarkMode(nativeElement, Bro1, Bro2, Bro3)
    }
...
function darkMode() {
   mode = "darkMode";
   document.body.style.backgroundColor = "var(--bodydark-bg)";
   h1.style.color = "#ffffff"
   playBtn.style.filter = "brightness(350%)";
   stopBtn.style.filter = "brightness(350%)";
   plusBtn.style.filter = "brightness(350%)";
   minusBtn.style.filter = "brightness(350%)";

   forestIco.style.filter = "brightness(400%)";
   rainIco.style.filter = "brightness(400%)";
   cafeIco.style.filter = "brightness(400%)";
   firesideIco.style.filter = "brightness(400%)";

   forestBtn.style.backgroundColor = "#29292E"
   rainBtn.style.backgroundColor = "#29292E"
   cafeBtn.style.backgroundColor = "#29292E"
   firesideBtn.style.backgroundColor = "#29292E"
...
function lightMode() {
   ...
   forestIco.style.filter = "none";
   rainIco.style.filter = "none";
   cafeIco.style.filter = "none";
   firesideIco.style.filter = "none";

   forestBtn.style.backgroundColor = "var(--sound-bg)";
   rainBtn.style.backgroundColor = "var(--sound-bg)";
   cafeBtn.style.backgroundColor = "var(--sound-bg)";
   firesideBtn.style.backgroundColor = "var(--sound-bg)";
...
lightOrDarkMode.addEventListener("click", () => {
    ...
    mode = "lightMode";
    lightMode()
    lightOrDarkMode.innerHTML = `<svg width="44" height="44" viewBox="0 0 44 44" fill="none" xmlns="http://www.w3.org/2000/svg">
...
forestBtn.addEventListener("click", () => {
    forestSoundOn()
    giveonlyOneSelectedSoundEnfasis(forestBtn, forestVolume, rainBtn, cafeBtn, firesideBtn) 
})

rainBtn.addEventListener("click", () => {
    rainSoundOn()
    giveonlyOneSelectedSoundEnfasis(rainBtn, rainVolume, forestBtn, cafeBtn, firesideBtn)
})

cafeBtn.addEventListener("click", () => {
    cafeSoundOn()
    giveonlyOneSelectedSoundEnfasis(cafeBtn, cafeVolume, forestBtn, rainBtn, firesideBtn)
})

firesideBtn.addEventListener("click", () => {
    firesideSoundOn()
    giveonlyOneSelectedSoundEnfasis(firesideBtn, firesideVolume, forestBtn, rainBtn, cafeBtn)
})


forestVolume.addEventListener("change", () => {
    volumeMedium(forestVolume, forest)
    volumeHigh(forestVolume, forest)
});

rainVolume.addEventListener("change", () => {
    volumeMedium(rainVolume, rain)
    volumeHigh(rainVolume, rain)
});

cafeVolume.addEventListener("change", () => {
    volumeMedium(cafeVolume, cafe)
    volumeHigh(cafeVolume, cafe)
});

firesideVolume.addEventListener("change", () => {
    volumeMedium(firesideVolume, fireside)
    volumeHigh(firesideVolume, fireside)
});